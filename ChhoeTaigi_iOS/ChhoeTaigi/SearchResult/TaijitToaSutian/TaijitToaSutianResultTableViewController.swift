...
class TaijitToaSutianResultTableViewController: UITableViewController {
    ...
    private var results: Results<TaijitToaSutianModel>?
    private var currentSelectedModel: TaijitToaSutianModel?
    ...
    func starSearch() {
        if (searchAllType == SearchAllViewController.SEARCH_ALL_TYPE_LOMAJI_SOOJI || searchAllType == SearchAllViewController.SEARCH_ALL_TYPE_LOMAJI) {
            ...
            self.results = RealmDatabaseLoader.getBundledRealm().objects(TaijitToaSutianModel.self).filter(predicate)
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "TableCell", for: indexPath) as! TaijitToaSutianResultTableViewCell
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if (segue.identifier == "segueWordView") {
            let vc = segue.destination as! TaijitToaSutianWordTableViewController
