...
class EmbreeTaigiSutianResultTableViewController: CustomBackButtonTableViewController {
    ...
    private var results: Results<EmbreeTaigiSutianModel>?
    private var currentSelectedModel: EmbreeTaigiSutianModel?
    ...
    func starSearch() {
        if (searchAllType == SearchAllViewController.SEARCH_ALL_TYPE_LOMAJI_SOOJI || searchAllType == SearchAllViewController.SEARCH_ALL_TYPE_LOMAJI) {
            ...
            self.results = RealmDatabaseLoader.getBundledRealm().objects(EmbreeTaigiSutianModel.self).filter(predicate)
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "TableCell", for: indexPath) as! EmbreeTaigiSutianResultTableViewCell
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if (segue.identifier == "segueWordView") {
            let vc = segue.destination as! EmbreeTaigiSutianWordTableViewController
