...
class KauiokpooTaigiDictResultTableViewController: CustomBackButtonTableViewController {
    ...
    private var results: Results<KauiokpooTaigiSutianModel>?
    private var currentSelectedModel: KauiokpooTaigiSutianModel?
    ...
    func setup() {
        self.titleLabel.text = "5. 教育部台語辭典: \(self.keyword ?? "")"
    ...
    func starSearch() {
        if (searchAllType == SearchAllViewController.SEARCH_ALL_TYPE_LOMAJI_SOOJI || searchAllType == SearchAllViewController.SEARCH_ALL_TYPE_LOMAJI) {
            ...
            self.results = RealmDatabaseLoader.getBundledRealm().objects(KauiokpooTaigiSutianModel.self).filter(predicate)
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "TableCell", for: indexPath) as! KauiokpooTaigiDictResultTableViewCell
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if (segue.identifier == "segueWordView") {
            let vc = segue.destination as! KauiokpooTaigiDictWordTableViewController
